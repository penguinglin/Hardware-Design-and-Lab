...
//  Signal Control                                      //
// ==================================================== //
// debounce and one pulse for play, right, left, forward
// module debounce (input wire clk,input wire pb, output wire pb_debounced );
// module one_pulse (input wire clk, input wire pb_in, output reg pb_out);
wire play_db, right_db, left_db, forward_db;
wire play_op, right_op, left_op, forward_op;
debounce play_db_inst (.clk(clk), .pb(play), .pb_debounced(play_db));
one_pulse play_op_inst (.clk(clk), .pb_in(play_db), .pb_out(play_op));
debounce right_db_inst (.clk(clk), .pb(right), .pb_debounced(right_db));
one_pulse right_op_inst (.clk(clk), .pb_in(right_db), .pb_out(right_op));
debounce left_db_inst (.clk(clk), .pb(left), .pb_debounced(left_db));
one_pulse left_op_inst (.clk(clk), .pb_in(left_db), .pb_out(left_op));
debounce forward_db_inst (.clk(clk), .pb(forward), .pb_debounced(forward_db));
one_pulse forward_op_inst (.clk(clk), .pb_in(forward_db), .pb_out(forward_op));
